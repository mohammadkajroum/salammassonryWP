<?php 
    $ed_shop_section = vidcast_get_option( 'enable_shop_section' );
    $section_title   = vidcast_get_option( 'shop_post_title' );
    $section_content = vidcast_get_option( 'shop_post_description' );
    $product_type    = vidcast_get_option( 'shop_select_product_from' );
    $custom_product  = vidcast_get_option( 'select_product_category' );
    $button_lbl      = vidcast_get_option( 'shop_post_button_text' );
    $button_link     = vidcast_get_option( 'shop_post_button_url' );
    $shop_number_of_column  = vidcast_get_option( 'shop_number_of_column' );
    $shop_number_of_product = vidcast_get_option( 'shop_number_of_product' );
    if( $ed_shop_section ){
                        ...
                        <ul class="woocommerce product-section-grid product-grid-<?php echo absint($shop_number_of_column);?>">
                            <?php while( $qry->have_posts() ){
                                ...
                                <li <?php wc_product_class('theme-product-item', $product); ?>>
                                    <div class="featured-media featured-media-radius theme-product-image">
                                        ...
                                        <a href="<?php the_permalink(); ?>" rel="bookmark">
                                            ...
                                            if (has_post_thumbnail()) {
                                                ...
                                                vidcast_theme_svg('vidcast-shop'); //fallback
                                            }
                                            ?>
                                        </a>
                                        <?php
                                        $stock = get_post_meta(get_the_ID(), '_stock_status', true);
                                        if ($stock == 'outofstock') {
                                            echo '<span class="outofstock-label">' . esc_html__('Sold Out', 'vidcast') . '</span>';
                                        } else {
