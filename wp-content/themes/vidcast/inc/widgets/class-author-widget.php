...
/**
 * Adds Vidcast_Author_Info widget.
 */
class Vidcast_Author_Info extends WP_Widget
{
    ...
    {
        parent::__construct(
            'vidcast_author_info_widget',
            esc_html__('Vidcast: Author Info', 'vidcast'),
            array('description' => esc_html__('Displays author short info.', 'vidcast'),)
        );

        $this->social_networks = apply_filters('vidcast_author_widget_social_networks', array(
            'facebook',
            'twitter',
            'linkedin',
            'instagram',
    ...
    public function widget($args, $instance)
    {
        ...
        do_action('vidcast_before_author_info');
        ?>
                    ...
                    if (!empty($social_networks) && is_array($social_networks)) {
                        foreach ($social_networks as $network) {
                            if (!empty($instance[$network])) {
                                $svg = Vidcast_SVG_Icons::get_theme_svg_name($instance[$network]);
                                if ($svg) {
                                    ?>
        ...
        do_action('vidcast_after_author_info');
    ...
    public function form($instance)
    {
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('author_desc')); ?>">
                <?php esc_attr_e('Short Description:', 'vidcast'); ?>
            </label>
        ...
        <div>
            <label for="<?php echo esc_attr($this->get_field_id('author_img')); ?>">
                <?php esc_attr_e('Author Image:', 'vidcast'); ?>
            </label>
            <?php
            ...
            <div class="image-field">
                ...
                <p>
                    ...
                    <button type="button" class="upload_image_button button" data-uploader-title-txt="<?php esc_attr_e('Use Image', 'vidcast'); ?>" data-uploader-btn-txt="<?php esc_attr_e('Choose an Image', 'vidcast'); ?>">
                        <?php _e('Upload/Add image', 'vidcast'); ?>
                    </button>
                    <button type="button" class="remove_image_button button" style="<?php echo esc_attr($remove_button_style); ?>"><?php _e('Remove image', 'vidcast'); ?></button>
                </p>
            </div>
        </div>
        <div>
            <p>
                ...
                <label for="<?php echo esc_attr($this->get_field_id('image_border_radius')); ?>">
                    <?php _e('Enable Imaged Border Radius', 'vidcast'); ?>
                </label>
            </p>
        </div>
        <div>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('style')); ?>">
                    <span class="field-label"><?php _e('Style', 'vidcast'); ?></span>
                </label>
                <select id="<?php echo esc_attr($this->get_field_id('style')); ?>"
                        name="<?php echo esc_attr($this->get_field_name('style')); ?>" class="widefat">
                    <option value="style_1" <?php selected('style_1', $style); ?>><?php _e('Style 1', 'vidcast'); ?></option>
                    <option value="style_2" <?php selected('style_2', $style); ?>><?php _e('Style 2', 'vidcast'); ?></option>
                    <option value="style_3" <?php selected('style_3', $style); ?>><?php _e('Style 3', 'vidcast'); ?></option>
                </select>
            </p>
        </div>
        <?php
