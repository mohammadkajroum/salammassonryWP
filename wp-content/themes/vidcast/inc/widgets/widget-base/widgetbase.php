 ...
 * @extends  WP_Widget
 */
abstract class Vidcast_Widget_Base extends WP_Widget {
    ...
    public function __construct() {
        ...
        add_action( 'admin_enqueue_scripts', array( $this, 'vidcast_widget_scripts' ) );
    ...
    public function update( $new_instance, $old_instance ) {
        ...
        foreach ( $this->settings as $key => $setting ) {
             ...
             */
            $instance[ $key ] = apply_filters( 'vidcast_widget_settings_sanitize', $instance[ $key ], $new_instance, $key, $setting );
        }
    ...
    public function form( $instance ) {
        ...
        foreach ( $this->settings as $key => $setting ) {
            ...
            switch ( $setting['type'] ) {
                            ...
                            <div class="image-field">
                                ...
                                <p>
                                    ...
                                    <button type="button" class="upload_image_button button" data-uploader-title-txt="<?php esc_attr_e( 'Use Image', 'vidcast' ); ?>" data-uploader-btn-txt="<?php esc_attr_e( 'Choose an Image', 'vidcast' ); ?>">
                                        <?php esc_html_e( 'Upload/Add image', 'vidcast' ); ?>
                                    </button>
                                    <button type="button" class="remove_image_button button" style="<?php echo esc_attr( $remove_button_style ); ?>"><?php esc_html_e( 'Remove image', 'vidcast' ); ?></button>
                                </p>
                            </div>
                        </div>
                        <?php
                ...
                default:
                    do_action( 'vidcast_widget_field_' . $setting['type'], $key, $value, $setting, $instance );
                    break;
            }
        }
    }
     ...
     * @return void
     */
    public function vidcast_widget_scripts( $hook ) {
        if ( 'widgets.php' === $hook ) {
            wp_enqueue_style( 'wp-color-picker' );
            wp_enqueue_media();
            wp_enqueue_style( 'vidcast_widgets_css', get_template_directory_uri() . '/inc/widgets/widget-base/css/style.css' );
            wp_enqueue_script( 'vidcast_widgets_js', get_template_directory_uri() . '/inc/widgets/widget-base/js/script.js', array( 'jquery', 'wp-color-picker' ), '1.0.0', true );
        }
    }
