 ...
 */
function vidcast_customize_register( $wp_customize ) {
	...
	$widgets_section->title = esc_html__( 'Widgets','vidcast' );
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial(
			...
			array(
				...
				'render_callback' => 'vidcast_customize_partial_blogname',
			)
		);
		$wp_customize->selective_refresh->add_partial(
			'blogdescription',
			array(
				...
				'render_callback' => 'vidcast_customize_partial_blogdescription',
			)
		);
	}

	/*Get default values to set while building customizer elements*/
	$default_options = vidcast_get_default_customizer_values();
	/* Header Background Color*/
	$wp_customize->add_setting(
	    'vidcast_options[header_bg_color]',
	    array(
	...
	$wp_customize->add_control(
	    new WP_Customize_Color_Control(
	        ...
	        'vidcast_options[header_bg_color]',
	        array(
	            'label' => __('Header Background Color', 'vidcast'),
	            'section' => 'colors',
	            'type' => 'color',
        		'priority' => 1,
	...
	$wp_customize->add_section( 'pro__section', array(
		'title'       => '' . esc_html__( 'View PRO Version', 'vidcast' ),
		'priority'    => 2,
		'description' => sprintf(
			...
			sprintf( '<a href="%1$s" target="_blank" class="button button-primary">%2$s</a>', esc_url( vidcast_get_pro_link() ), esc_html__( 'View Vidcast PRO', 'vidcast' ) )
		),
	) );
	$wp_customize->add_setting( 'vidcast_style_view_pro_desc', array(
		'default'           => '',
		'sanitize_callback' => '__return_true',
	) );
	$wp_customize->add_control( 'vidcast_style_view_pro_desc', array(
		'section' => 'pro__section',
		'type'    => 'hidden',
	) );
}
add_action( 'customize_register', 'vidcast_customize_register' );
 ...
 * @return void
 */
function vidcast_customize_partial_blogname() {
	bloginfo( 'name' );
}
 ...
 * @return void
 */
function vidcast_customize_partial_blogdescription() {
 ...
 */
function vidcast_customize_preview_js() {
    wp_enqueue_script( 'vidcast-customizer', get_template_directory_uri() . '/assets/js/customizer.js', array( 'customize-preview' ), VIDCAST_S_VERSION, true );

}
add_action( 'customize_preview_init', 'vidcast_customize_preview_js' );
 ...
 * @since 1.0.0
 */
function vidcast_customizer_control_scripts(){
    wp_enqueue_style('vidcast-customizer-css', get_template_directory_uri() . '/assets/css/customizer.css');
    wp_enqueue_script('vidcast-customizer-controls', get_template_directory_uri() . '/assets/js/customizer-admin.js', array('jquery', 'jquery-ui-sortable', 'customize-controls') );
}
add_action('customize_controls_enqueue_scripts', 'vidcast_customizer_control_scripts', 0);

/**
 * Generate a link to the premium theme info page.
 */
function vidcast_get_pro_link() {
	return 'https://www.themeinwp.com/theme/vidcast-pro/';
}