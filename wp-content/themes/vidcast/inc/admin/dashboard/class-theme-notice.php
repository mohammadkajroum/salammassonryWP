 ...
 */
add_action('admin_enqueue_scripts', 'vidcast_widgets_backend_enqueue');
function vidcast_widgets_backend_enqueue()
{
    wp_register_script('vidcast-custom-widgets', get_template_directory_uri() . '/inc/admin/dashboard/js/theme-notice.js', array('jquery'), true);
    $translation = array(
        'btn_text' => esc_html__('Processing...', 'vidcast'),
        'nonce' => wp_create_nonce('vidcast_demo_import_nonce'),
        'noncen' => 'vidcast_demo_import_nonce',
        'adminurl' => esc_url(admin_url()),
    );
    wp_localize_script('vidcast-custom-widgets', 'vidcast_adi_install', $translation);
    wp_enqueue_media();
    wp_enqueue_script('vidcast-custom-widgets');
}

/**
 * Class to handle notices and Advanced Demo Import
 *
 * Class Vidcast_Notice_Handler
 */
class Vidcast_Notice_Handler
{
    ...
    public function __construct()
    {
        ...
        $this->demo_url = 'https://live-demo.themeinwp.net/vidcast';
        ...
        if ('index.php' === $pagenow || 'plugins.php' === $pagenow) {
            ...
            add_action('admin_notices', array($this, 'vidcast_theme_info_welcome_admin_notice'), 3);
        }
        add_action('wp_ajax_vidcast_getting_started', array($this, 'vidcast_getting_started'));
    }

    /**
     * Display an admin notice linking to the about page
     */
    public function vidcast_theme_info_welcome_admin_notice()
    {
        if (is_admin() && !get_user_meta(get_current_user_id(), 'gs_notice_dismissed')) {
            echo '<div class="notice notice-success vidcast-notice is-dismissible getting-started">';
            if (is_plugin_active('mailchimp-for-wp/mailchimp-for-wp.php')) {
                echo('<div class="fs-notice success"><p>' . esc_html__('Plugins added successfully. You may dismiss this notice now.', 'vidcast') . '</p></div>');
            } else { ?>
                <p class="notice-text">
                    <?php
                ...
                <div class="theme-admin-button-wrap theme-admin-button-group">
                    <a href="<?php echo esc_url($this->upgrade_url);?>" target="_blank" class="button button-primary theme-admin-button admin-button-secondary">
                        ...
                        <span><?php echo esc_html__('Upgrade to Pro', 'vidcast');?></span>
                    </a>
                    <a href="<?php echo admin_url('themes.php?page=theme-dashboard'); ?>" class="button theme-admin-button admin-button-primary">
                        <span class="dashicons dashicons-awards"></span>
                        <span><?php echo sprintf(esc_html__('Get started with %s', 'vidcast'), $this->theme_name); ?></span>
                    </a>
                    <a href="#" class="vidcast-install-plugins button theme-admin-button admin-button-primary">
                        <span class="dashicons dashicons-admin-plugins"></span>
                        <span><?php echo esc_html__('Install and activate the recommended plugins', 'vidcast'); ?></span>
                    </a>
                    <a href="<?php echo esc_url($this->demo_url);?>" target="_blank" class="button theme-admin-button admin-button-secondary">
                        <span class="dashicons dashicons-welcome-view-site"></span>
                        <span><?php echo esc_html__('View Demo', 'vidcast');?></span>
                    </a>
                </div>
     ...
     * return void
     */
    public function vidcast_getting_started()
    {

        check_ajax_referer('vidcast_demo_import_nonce', 'security');
        ...
        if (!current_user_can('install_plugins')) {
            $status['errorMessage'] = __('Sorry, you are not allowed to install plugins on this site.', 'vidcast');
        ...
        if (is_wp_error($result)) {
            ...
            $status['errorMessage'] = __('Unable to connect to the filesystem. Please confirm your credentials.', 'vidcast');
