( function( $ ) {
    ...
    $('.twp-img-upload-button').click(function () {
        ...
        frame = wp.media({
            title: vidcast_localize.upload_image,
            button: {
                text: vidcast_localize.use_image
            },
    ...
    $(document).on('click', '.vidcast-primary-notice .notice-dismiss', function (event) {
        event.preventDefault();
        jQuery.post(vidcast_localize.ajax_url, {
            action: 'vidcast_dismissed_handler',
            security : vidcast_localize.dismiss_notice_nonce,
            notice: $(this).closest('.vidcast-primary-notice').data('notice'),
        });
