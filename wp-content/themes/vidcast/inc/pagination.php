...
if ( ! function_exists( 'vidcast_ajax_pagination' ) ) :
    /**
     ...
     */
    function vidcast_ajax_pagination($type) {
        global $wp_query; 
        if ( $wp_query->max_num_pages > 1  ) {

            ?>
            <div class="vidcast-load-posts-btn-wrapper" data-page="1" data-max-pages="<?php echo esc_attr($wp_query->max_num_pages);?>" data-load-type="<?php echo esc_attr($type);?>">
                <hr>
                <a href="#" class="ajax-viewmore-link">
                    <span class="vidcast-ajax-loader"></span>
                    <button class="load-btn"><?php _e('Load More', 'vidcast') ?></button>
                </a>
            </div>
            <?php
        }
...
if ( ! function_exists( 'vidcast_load_posts' ) ) :
    /**
     ...
     *
     */
    function vidcast_load_posts() {

        check_ajax_referer( 'vidcast-load-posts-nonce', 'nonce' );
        ...
        if($posts->have_posts()):
            ...
            $error = new WP_Error( '500', __('No More Posts','vidcast') );
...
add_action( 'wp_ajax_vidcast_load_posts', 'vidcast_load_posts' );
add_action( 'wp_ajax_nopriv_vidcast_load_posts', 'vidcast_load_posts' );